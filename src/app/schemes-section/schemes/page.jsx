...
const Schemes = () => {
  ...
  const exploreSchemesSectionRef = useRef(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const genAI = new GoogleGenerativeAI("AIzaSyD-Dj8nuhXpFkWmSUvv6AAy_frXYeOW87U");
  ...
  const handleSearch = (e) => {
    ...
    if (query.trim() === '') {
      // If search is empty, reset to show all schemes based on active category
      if (activeCategory === "All Categories") {
        setFilteredSchemes(schemes);
      } else {
        const mappedCategory = categoryMappings[activeCategory];
  ...
  const handleKeyPress = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      // Scroll to the explore schemes section
      exploreSchemesSectionRef.current?.scrollIntoView({ 
        behavior: 'smooth',
        block: 'start'
      });
    }
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-base font-medium text-gray-900">
                    {userProfile ? "View Your Liked Schemes" : "Explore Liked Schemes"}
                  </h3>
                  <p className="text-gray-500 text-sm">Visit Dashboard</p>
                </div>
              </div>
